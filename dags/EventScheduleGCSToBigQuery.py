...
def event_schedule_gcs_to_bigquery():
    ...
    # CSV 파일이 저장된 GCS 경로
    gcs_uri = 'gs://flower-pipeline-bucket/academic_calendar/event_list.csv'
...
default_args = {
    ...
    'schedule_interval' : '0 6 1 * *',  # 매달 1일 2시에 실행
    'retries': 1,
...
with DAG(
    'event_schedule_gcs_to_bigquery_dag',
    default_args=default_args,
    schedule_interval=timedelta(days=1),
) as dag:

    load_to_bigquery = PythonOperator(
        task_id='event_schedule_gcs_to_bigquery',
        python_callable=event_schedule_gcs_to_bigquery,
    )